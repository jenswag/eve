...
class HANNA_Ensemble(nn.Module):
    def __init__(self, model_paths, model_class, Embedding_ChemBERT=384, nodes=96, device=None):
        super(HANNA_Ensemble, self).__init__()
        self.models = nn.ModuleList()
        self.device = device if device else torch.device('cpu')
        for path in model_paths:
            # Load the model state dict
            model_state = torch.load(path, map_location=self.device)
            # Initialize a new HANNA model instance with required arguments
            model = model_class(Embedding_ChemBERT=Embedding_ChemBERT, nodes=nodes)
